    ...
    while (1) {
        ...
        if (rc > 0) {
            ...
            if (method == Lookup) {
                ...
                //printf("At server pt B Method Lookup \n);
                int status = mfs_lookup(request.pinum, request.name);

                printf("Method request name %s status %d \n", request.name, status );
                char response_buffer[BUFFER_SIZE];
                memcpy(response_buffer, &status, sizeof(int));
                #if 0
                rc = UDP_Write(sd, &addr, response_buffer, BUFFER_SIZE);
                #endif
                rc = UDP_Write(sd, &addr, response_buffer, sizeof(int));
                printf("after UDP_Write rc %d \n", rc );
            } else if (method == Stat) {
                printf("method stat \n" );
                ...
                printf("method_unlink status %d pinum %d name %s\n", 
                       status, request.pinum, request.name);
